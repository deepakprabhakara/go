...
func (c *RandomLitecoinClient) Start(addresses <-chan string) {
	c.log = common.CreateLogger("RandomLitecoinClient")
...
func (g *RandomLitecoinClient) logStats() {
	for {
...
func (g *RandomLitecoinClient) addUserAddresses(addresses <-chan string) {
	for {
		address := <-addresses
...
func (c *RandomLitecoinClient) GetBlockCount() (int64, error) {
	return c.currentBlockNumber, nil
}

// This should always return testnet genesis block hash for `0` so production
// litecoin.Listener genesis block check will fail.
func (c *RandomLitecoinClient) GetBlockHash(blockHeight int64) (*chainhash.Hash, error) {
	if blockHeight == 0 {
		return chaincfg.TestNet4Params.GenesisHash, nil
	}
...
func (c *RandomLitecoinClient) GetBlock(blockHash *chainhash.Hash) (*wire.MsgBlock, error) {
...
func (c *RandomLitecoinClient) generateBlocks() {
	for {
		block := &wire.MsgBlock{
...
func (g *RandomLitecoinClient) randomAddress() ltcutil.Address {
	g.userAddressesLock.Lock()
	...
	if len(g.userAddresses) > 0 {
		address, err = ltcutil.DecodeAddress(g.userAddresses[0], &chaincfg.TestNet4Params)
		if err != nil {
			panic(err)
		}
		...
		address, err = ltcutil.NewAddressPubKey(pubKey, &chaincfg.TestNet4Params)
		if err != nil {
			panic(err)
		}
	}
...
// randomAmount generates random amount between [0, 100) LTC
func (g *RandomLitecoinClient) randomAmount() int64 {
	return rand.Int63n(100 * litsInLtc)
}

func (g *RandomLitecoinClient) randomHash() string {
	var hash [32]byte
	rand.Read(hash[:])
