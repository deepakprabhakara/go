...
// onNewLitecoinTransaction checks if transaction is valid and adds it to
...
// queue.Transaction (especially amounts). Pooling service should not have to deal with any
// conversions.
//
...
func (s *Server) onNewLitecoinTransaction(transaction litecoin.Transaction) error {
	...
	if transaction.ValueLit < s.minimumValueLit {
		localLog.Debug("Value is below minimum required amount, skipping")
		return nil
	}

	addressAssociation, err := s.Database.GetAssociationByChainAddress(database.ChainLitecoin, transaction.To)
	if err != nil {
	...
	processed, err := s.Database.AddProcessedTransaction(database.ChainLitecoin, transaction.Hash, transaction.To)
	if err != nil {
		return err
	}
